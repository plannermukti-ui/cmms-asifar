...
@section('title', 'Edit Laporan Produksi Harian - CMMS Aisfar')
        ...
        <div class="row g-2 align-items-center">
            <div class="col">
                <h2 class="page-title text-uppercase font-weight-bold d-flex align-items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon text-warning" width="28" height="28" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M4 20h4l10.5 -10.5a2.828 2.828 0 1 0 -4 -4l-10.5 10.5v4" /><path d="M13.5 6.5l4 4" /></svg>
                    Edit Laporan Produksi Harian (Shift)
                </h2>
                <div class="text-secondary mt-1">Perbarui data ritasi armada, jam kerja unit support, dan catatan kendala delay.</div>
            </div>
...
<div class="page-body">
    <div class="container-xl">
        <form action="{{ route('productions.update', $production) }}" method="POST" id="productionForm">
            @csrf
            @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="shift" class="form-select" required>
                                <option value="DS" {{ $production->shift == 'DS' ? 'selected' : '' }}>Day Shift (DS - Siang)</option>
                                <option value="NS" {{ $production->shift == 'NS' ? 'selected' : '' }}>Night Shift (NS - Malam)</option>
                            </select>
                        </div>
            ...
            <div class="mb-5 d-flex justify-content-end">
                <button type="submit" class="btn btn-primary btn-lg shadow-sm d-flex align-items-center gap-2">
                    ...
                    Simpan Perubahan Laporan Shift
                </button>
            </div>
        </form>
    </div>
...
<script>
    ...
    function addHauler(fleetId, initialData = null) {
        ...
        for(let i=1; i<=12; i++) {
            ...
            hourlyInputs += `<td class="p-1"><input type="number" min="0" name="fleets[${fleetId}][haulers][${hId}][hourly_ritasi][${i}]" class="form-control ritasi-input text-center" placeholder="0" value="${hourlyVal}"></td>`;
        }
        ...
        tr.innerHTML = `
            <td>${selectHtml}</td>
            <td><input type="number" step="0.01" name="fleets[${fleetId}][haulers][${hId}][payload]" class="form-control form-control-sm text-center" placeholder="20" required value="${payloadVal}"></td>
            ${hourlyInputs}
    ...
    function addSupport(initialData = null) {
        ...
        tr.innerHTML = `
            <td>${selectHtml}</td>
            <td><input type="number" step="0.01" name="supports[${supportIndex}][hm_awal]" class="form-control form-control-sm" placeholder="0.00" required value="${hmAwal}"></td>
            <td><input type="number" step="0.01" name="supports[${supportIndex}][hm_akhir]" class="form-control form-control-sm" placeholder="0.00" required value="${hmAkhir}"></td>
    ...
    function addDelay(initialData = null) {
        ...
        tr.innerHTML = `
            <td><input type="time" name="delays[${currDelayIdx}][start_time]" class="form-control form-control-sm" required value="${startVal}"></td>
            <td><input type="time" name="delays[${currDelayIdx}][end_time]" class="form-control form-control-sm" required value="${endVal}"></td>
            <td>
            ...
            <td>
                <select name="delays[${currDelayIdx}][fleet_id]" class="form-select form-select-sm delay-fleet-select" data-selected="${mappedFleetIdx}">
                    <option value="">-- Semua Fleet (Global) --</option>
                </select>
            </td>
            <td><input type="text" name="delays[${currDelayIdx}][remarks]" class="form-control form-control-sm" placeholder="Keterangan..." value="${rmks}"></td>
            <td class="text-center"><button type="button" class="btn btn-icon btn-sm btn-ghost-danger shadow-none" onclick="this.closest('tr').remove()" title="Hapus"><svg xmlns="http://www.w3.org/2000/svg" class="icon m-0" width="18" height="18" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg></button></td>
        `;
