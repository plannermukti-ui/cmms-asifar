...
@section('title', 'Edit Failure Analysis Report')
...
<div class="container-xl">
        ...
        <div class="row align-items-center">
            <div class="col">
                ...
                <h2 class="page-title text-primary fw-bold">
                    Edit Failure Analysis Report ({{ $far->no_far }})
                </h2>
            </div>
            <div class="col-auto ms-auto d-print-none">
                <a href="{{ route('fars.show', $far) }}" class="btn btn-outline-secondary">
                    Kembali
                </a>
            </div>
        </div>
    ...
    <form action="{{ route('fars.update', $far) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="row row-cards">
            ...
            <div class="col-12">
                <div class="card shadow-sm rounded-4 border-0">
                    <div class="card-header bg-primary text-white rounded-top-4">
                        <h3 class="card-title text-white">Informasi Dasar</h3>
                    </div>
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-md-3">
                                <label class="form-label required">No. FAR</label>
                                <input type="text" name="no_far" class="form-control fw-bold text-primary" value="{{ old('no_far', $far->no_far) }}" required readonly>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label required">Site / Project</label>
                                <select name="site_id" class="form-select" required>
                                    @foreach($sites as $site)
                                        <option value="{{ $site->id }}" {{ old('site_id', $far->site_id) == $site->id ? 'selected' : '' }}>{{ $site->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                                ...
                                <select name="work_order_id" id="work_order_id" class="form-select">
                                    ...
                                    @foreach($workOrders as $wo)
                                        <option value="{{ $wo->id }}" data-unit="{{ $wo->master_unit_id }}" data-site="{{ $wo->site_id }}" {{ old('work_order_id', $far->work_order_id) == $wo->id ? 'selected' : '' }}>{{ $wo->no_wo }} - {{ $wo->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                                ...
                                <select name="master_unit_id" id="master_unit_id" class="form-select" required>
                                    ...
                                    @foreach($units as $unit)
                                        <option value="{{ $unit->id }}" data-model="{{ $unit->model->name ?? '' }}" data-sn="{{ $unit->sn_chassis }}" data-engmodel="{{ $unit->engine_model }}" data-engsn="{{ $unit->sn_engine }}" {{ old('master_unit_id', $far->master_unit_id) == $unit->id ? 'selected' : '' }}>{{ $unit->nomor_unit }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="date" name="date_reported" class="form-control" value="{{ old('date_reported', $far->date_reported) }}" required>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label required">Tanggal Kejadian (Failure)</label>
                                <input type="date" name="date_of_failure" class="form-control" value="{{ old('date_of_failure', $far->date_of_failure) }}" required>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">SMU Saat Kejadian</label>
                                <input type="number" name="smu_at_failure" class="form-control" value="{{ old('smu_at_failure', $far->smu_at_failure) }}">
                            </div>
                        </div>

                        <div class="row g-3 mt-2 bg-light p-3 rounded" id="unitInfoBox">
                            <div class="col-md-3">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="component_part_no" id="component_part_no" class="form-select tom-select" data-placeholder="Pilih Part Number...">
                                ...
                                @foreach($parts as $part)
                                    <option value="{{ $part->part_number }}" data-desc="{{ $part->part_description }}" {{ old('component_part_no', $far->component_part_no) == $part->part_number ? 'selected' : '' }}>{{ $part->part_number }} - {{ $part->part_description }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Nama Komponen (Description)</label>
                            <input type="text" name="component_description" id="component_description" class="form-control" value="{{ old('component_description', $far->component_description) }}">
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="part_no_causing_failure" id="part_no_causing_failure" class="form-select tom-select" data-placeholder="Pilih Part Number...">
                                <option value=""></option>
                                @foreach($parts as $part)
                                    <option value="{{ $part->part_number }}" {{ old('part_no_causing_failure', $far->part_no_causing_failure) == $part->part_number ? 'selected' : '' }}>{{ $part->part_number }} - {{ $part->part_description }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="card shadow-sm rounded-4 border-0 mb-3">
                    ...
                    <div class="card-body row g-3">
                        <div class="col-md-4">
                            ...
                            <input type="date" name="last_comp_date" class="form-control" value="{{ old('last_comp_date', $far->last_comp_date) }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">SMU Pasang</label>
                            <input type="number" name="last_comp_smu" class="form-control" value="{{ old('last_comp_smu', $far->last_comp_smu) }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Umur Komp.</label>
                            <input type="number" name="hours_of_component" class="form-control" value="{{ old('hours_of_component', $far->hours_of_component) }}">
                        </div>
                    </div>
                </div>
                <div class="card shadow-sm rounded-4 border-0">
                    ...
                    <div class="card-body row g-3">
                        <div class="col-md-3">
                            ...
                            <input type="date" name="last_oil_date_taken" class="form-control" value="{{ old('last_oil_date_taken', $far->last_oil_date_taken) }}">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Tgl Dikirim</label>
                            <input type="date" name="last_oil_date_sent" class="form-control" value="{{ old('last_oil_date_sent', $far->last_oil_date_sent) }}">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Tgl Diterima</label>
                            <input type="date" name="last_oil_date_received" class="form-control" value="{{ old('last_oil_date_received', $far->last_oil_date_received) }}">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Evaluasi</label>
                            <select name="last_oil_eval" class="form-select">
                                <option value="">-Pilih-</option>
                                <option value="A" {{ old('last_oil_eval', $far->last_oil_eval) == 'A' ? 'selected' : '' }}>A</option>
                                <option value="B" {{ old('last_oil_eval', $far->last_oil_eval) == 'B' ? 'selected' : '' }}>B</option>
                                <option value="C" {{ old('last_oil_eval', $far->last_oil_eval) == 'C' ? 'selected' : '' }}>C</option>
                                <option value="X" {{ old('last_oil_eval', $far->last_oil_eval) == 'X' ? 'selected' : '' }}>X</option>
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-12 mt-3">
                <div class="card shadow-sm rounded-4 border-0">
                    ...
                    <div class="card-body">
                        <div class="mb-4">
                            <label class="form-label fw-bold">1. Failure Outline</label>
                            <textarea name="failure_outline" class="form-control" rows="4">{{ old('failure_outline', $far->failure_outline) }}</textarea>
                        </div>
                        <div class="mb-4">
                            <label class="form-label fw-bold">2. Background</label>
                            <textarea name="background" class="form-control" rows="4">{{ old('background', $far->background) }}</textarea>
                        </div>
                        <div class="mb-4">
                            <label class="form-label fw-bold">3. Failure Analysis</label>
                            <textarea name="failure_analysis" class="form-control" rows="5">{{ old('failure_analysis', $far->failure_analysis) }}</textarea>
                        </div>
                        <div class="mb-4">
                            <label class="form-label fw-bold text-danger">4. Conclusion</label>
                            <textarea name="conclusion" class="form-control" rows="4">{{ old('conclusion', $far->conclusion) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-12 mt-3">
                <div class="card shadow-sm rounded-4 border-0">
                    ...
                    <div class="card-body">
                                    ...
                                    <div class="col-md-4">
                                        <div class="card shadow-none border">
                                            <div class="card-body p-2 text-center">
                                                <img src="{{ Storage::url($attach->photo_path) }}" class="img-fluid rounded mb-2" style="max-height:150px; object-fit:cover;">
                                                <div class="small fw-bold">{{ $attach->component }}</div>
                                                <div class="small text-muted text-truncate">{{ $attach->observation }}</div>
                                                <div class="mt-2">
                                                    <label class="form-check form-check-inline text-danger">
                                                        <input class="form-check-input" type="checkbox" name="delete_attachments[]" value="{{ $attach->id }}">
                                                        <span class="form-check-label small">Hapus Foto Ini</span>
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <hr>
                        <h4 class="mb-3">Upload Foto Baru:</h4>
                        <div id="attachmentContainer">
                            <!-- Template for new attachment row -->
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-12 text-end mt-4 mb-4">
                <button type="submit" class="btn btn-primary btn-lg px-5">Simpan Perubahan Laporan FAR</button>
            </div>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let attachmentIndex = 100; // start high to avoid collision
    ...
    btnAddAttachment.addEventListener('click', function() {
        ...
        <div class="row g-3 attachment-row mb-4 pb-3 border-bottom">
            ...
            <div class="col-md-4">
                ...
                <button type="button" class="btn btn-sm btn-outline-danger mt-2 btn-remove-row">Batalkan</button>
            </div>
        </div>`;
