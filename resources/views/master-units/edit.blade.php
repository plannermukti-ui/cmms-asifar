...
@section('title', 'Edit Master Unit - CMMS Aisfar')

@section('content')
<div class="page-header d-print-none">
  <div class="row align-items-center">
    <div class="col">
      <h2 class="page-title">Edit Master Unit: {{ $masterUnit->nomor_unit }}</h2>
    </div>
...
<div class="card mt-3">
  <form action="{{ route('master-units.update', $masterUnit) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="card-body">
      <div class="row row-cards">
        ...
        <div class="col-md-4">
          <div class="mb-3">
            ...
            <input type="text" name="nomor_unit" class="form-control" placeholder="A-07, MDT006, dll" value="{{ old('nomor_unit', $masterUnit->nomor_unit) }}" required>
          </div>
          <div class="mb-3">
            ...
            <div class="input-group">
              <select name="unit_type_id" id="unit_type_id" class="form-select" required>
                ...
                @foreach($unitTypes as $type)
                  <option value="{{ $type->id }}" {{ old('unit_type_id', $masterUnit->unit_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                @endforeach
              </select>
            ...
            <div class="input-group">
              <select name="unit_model_id" id="unit_model_id" class="form-select">
                ...
                @foreach($unitModels as $model)
                  <option value="{{ $model->id }}" data-type-id="{{ $model->unit_type_id }}" {{ old('unit_model_id', $masterUnit->unit_model_id) == $model->id ? 'selected' : '' }}>{{ $model->name }}</option>
                @endforeach
              </select>
          ...
          <div class="mb-3">
            ...
            <select name="site_id" class="form-select" required>
              ...
              @foreach ($sites as $site)
                <option value="{{ $site->id }}" {{ old('site_id', $masterUnit->site_id) == $site->id ? 'selected' : '' }}>{{ $site->name }}</option>
              @endforeach
            </select>
          </div>
          @endif
          <div class="mb-3">
            ...
            <input type="text" name="location" class="form-control" value="{{ old('location', $masterUnit->location) }}">
          </div>
          <div class="mb-3">
            <label class="form-label">Asal Unit (Dari)</label>
            <input type="text" name="dari" class="form-control" value="{{ old('dari', $masterUnit->dari) }}">
          </div>
          <div class="mb-3">
            <label class="form-label">Tanggal Diterima</label>
            <input type="date" name="date_receive" class="form-control" value="{{ old('date_receive', $masterUnit->date_receive) }}">
          </div>
        </div>
        ...
        <div class="col-md-4">
          <div class="mb-3">
            ...
            <input type="text" name="sn_chassis" class="form-control" value="{{ old('sn_chassis', $masterUnit->sn_chassis) }}">
          </div>
          <div class="mb-3">
            <label class="form-label">Model Mesin</label>
            <input type="text" name="engine_model" class="form-control" value="{{ old('engine_model', $masterUnit->engine_model) }}">
          </div>
          <div class="mb-3">
            <label class="form-label">SN Engine</label>
            <input type="text" name="sn_engine" class="form-control" value="{{ old('sn_engine', $masterUnit->sn_engine) }}">
          </div>
          <div class="mb-3">
            <label class="form-label">Merek Mesin (Engine Make)</label>
            <input type="text" name="engine_make" class="form-control" value="{{ old('engine_make', $masterUnit->engine_make) }}">
          </div>
          <div class="mb-3">
            <label class="form-label">Tahun Perakitan</label>
            <input type="text" name="perakitan" class="form-control" placeholder="YYYY" value="{{ old('perakitan', $masterUnit->perakitan) }}">
          </div>
          <div class="mb-3">
            <label class="form-label">Kapasitas</label>
            <input type="text" name="capacity" class="form-control" value="{{ old('capacity', $masterUnit->capacity) }}">
          </div>
          <div class="mb-3">
            <label class="form-label">Horse Power (HP)</label>
            <input type="text" name="hp" class="form-control" value="{{ old('hp', $masterUnit->hp) }}">
          </div>
        </div>
        ...
        <div class="col-md-4">
          <div class="mb-3">
            ...
            <input type="text" name="kw" class="form-control" value="{{ old('kw', $masterUnit->kw) }}">
          </div>
          <div class="mb-3">
            <label class="form-label">Nomor Polisi</label>
            <input type="text" name="no_polisi" class="form-control" value="{{ old('no_polisi', $masterUnit->no_polisi) }}">
          </div>
          <div class="mb-3">
            <label class="form-label">Attachments/Perlengkapan</label>
            <input type="text" name="attachments" class="form-control" value="{{ old('attachments', $masterUnit->attachments) }}">
          </div>
          <div class="mb-3">
            <label class="form-label">Remarks (Keterangan)</label>
            <textarea name="remarks" class="form-control" rows="3">{{ old('remarks', $masterUnit->remarks) }}</textarea>
          </div>
          <div class="mb-3">
            <div class="form-label">Status Service</div>
            <label class="form-check form-switch">
              <input class="form-check-input" type="checkbox" name="service" value="1" {{ old('service', $masterUnit->service) ? 'checked' : '' }}>
          ...
          <div class="mb-3">
            ...
            <label class="form-check form-switch">
              <input class="form-check-input" type="checkbox" name="active" value="1" {{ old('active', $masterUnit->active) ? 'checked' : '' }}>
