...
@section('title', 'Edit Hour Meter - CMMS Aisfar')

@section('content')
<div class="page-header d-print-none">
  <div class="row align-items-center">
    <div class="col">
      <h2 class="page-title">Edit Hour Meter</h2>
    </div>
...
<div class="card mt-3">
  <div class="card-body">
    <form action="{{ route('hour-meters.update', $hourMeter) }}" method="post">
      @csrf
      @method('PUT')
      <div class="row">
        <div class="col-md-4 mb-3">
          <label class="form-label required">Date</label>
          <input type="date" class="form-control" name="date" value="{{ old('date', $hourMeter->date) }}" required>
        </div>
        <div class="col-md-4 mb-3">
          ...
          <select name="master_unit_id" id="master_unit_id" class="form-select" required>
            ...
            @foreach($masterUnits as $unit)
              <option value="{{ $unit->id }}" 
                      ...
                      {{ old('master_unit_id', $hourMeter->master_unit_id) == $unit->id ? 'selected' : '' }}>
                {{ $unit->nomor_unit }}
              </option>
            @endforeach
          </select>
        ...
        <div class="col-md-4 mb-3">
          ...
          <input type="number" class="form-control" name="hm" step="0.1" min="0" value="{{ old('hm', $hourMeter->hm) }}" required>
        </div>
      </div>
      <div class="mt-4">
        <button type="submit" class="btn btn-primary">Update Hour Meter</button>
      </div>
    </form>
  </div>
</div>
